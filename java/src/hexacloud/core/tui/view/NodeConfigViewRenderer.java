...
public class NodeConfigViewRenderer {
    ...
    public void draw(TuiFrameBuffer frameBuffer) {
        ...
        frameBuffer.printAt(4, 6, WHITE_BOLD + "Host:   " + RESET + node.host());
        frameBuffer.printAt(4, 7, WHITE_BOLD + "Port:   " + RESET + node.port());
        ...
        frameBuffer.printAt(4, 8, WHITE_BOLD + "Status: " + RESET + coloredStatus);
        ...
        frameBuffer.printAt(4, 9, (canEdit ? WHITE_BOLD + "[P] " : "") + "Ping Proto: " + RESET + protocolColor + node.pingProtocol() + RESET);
        frameBuffer.printAt(4, 10, (canEdit ? WHITE_BOLD + "[E] " : "") + "Path/Route: " + RESET + CYAN + node.pingPath() + RESET);
        ...
        frameBuffer.printAt(4, 11, (canEdit ? WHITE_BOLD + "[H] " : "") + "Header: " + RESET + CYAN + headerName + RESET);
        frameBuffer.printAt(4, 12, (canEdit ? WHITE_BOLD + "[V] " : "") + "Token:  " + RESET + CYAN + headerVal + RESET);
        frameBuffer.printAt(4, 13, "Mutability: " + (node.isDynamic() ? YELLOW + "DYNAMIC" : GREEN + "STATIC") + RESET);

        // Sep & Right half: Live Telemetry
        int xSep = W / 2;
        int xTele = xSep + 2;
        for (int row = 6; row <= 13; row++) {
            frameBuffer.printAt(xSep, row, "│");
        }
        frameBuffer.printAt(xTele, 6, WHITE_BOLD + "Live Telemetry Metrics:" + RESET);
        String runtimeDisplay = node.runtime().isEmpty() ? node.pingProtocol().getFriendlyName() : node.runtime();
        frameBuffer.printAt(xTele, 7, "Runtime/Lang: " + CYAN + runtimeDisplay + RESET);
        
        String latencyStr = node.status().name().equals("ONLINE") ? node.latencyMs() + " ms" : "-";
        frameBuffer.printAt(xTele, 8, "Latency(RTT): " + GREEN + latencyStr + RESET);
        
        String cpuStr = node.status().name().equals("ONLINE") ? String.format("%.1f %%", node.cpuUsage()) : "-";
        frameBuffer.printAt(xTele, 9, "CPU Load:     " + YELLOW + cpuStr + RESET);
        
        String ramStr = node.status().name().equals("ONLINE") ? String.format("%.1f MB", node.ramUsage()) : "-";
        frameBuffer.printAt(xTele, 10, "RAM Memory:   " + CYAN + ramStr + RESET);

        // Bottom half: console logs for this node/service
        mainRenderer.drawBox(frameBuffer, 2, 15, W, H - 2, "CONSOLE LOGS FOR SERVICE " + node.getFullHost(), false);
        ...
        if (serviceLogs.isEmpty()) {
            frameBuffer.printAt(4, y, "No logs recorded for this service.");
            y++;
        } else {
